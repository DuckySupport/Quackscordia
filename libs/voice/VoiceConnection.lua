...
function VoiceConnection:_createAudioPacket(opus_data, opus_len, ssrc, key)
    ...
    -- Fix: Ensure ssrc is treated as an unsigned 32-bit integer to prevent overflow
    -- `bit.band(ssrc, 0xFFFFFFFF)` effectively masks the ssrc to fit within 32 bits
    -- if it somehow received a larger value, preventing the 'unsigned overflow' error.
    local header = pack(HEADER_FMT, 0x80, 0x78, s, t, bit.band(ssrc, 0xFFFFFFFF))
